...
export default function HomePage({ user, tokens, setTokens, bets, setBets, showUserGuideModal, setShowUserGuideModal, userGuideContent }) {
  ...
  const filteredCompanies = selectedDomain === 'all' 
    ? mockCompanies.filter(c => !hasResultsDeclared(c))
    : mockCompanies.filter(c => c.liveEventSection === selectedDomain && !hasResultsDeclared(c));
  ...
  const hotBetsCompanies = (selectedDomain === 'all'
    ...
    .sort((a, b) => {
      ...
      const totalTokensA = mockBets.filter(bet => bet.companyId === a.id)
        .reduce((sum, bet) => sum + (Number(bet.amount) || 0), 0);
      const totalTokensB = mockBets.filter(bet => bet.companyId === b.id)
        .reduce((sum, bet) => sum + (Number(bet.amount) || 0), 0);
  ...
  // Use admin-selected companies for the carousel
  const adminCarouselCompanies = mockCompanies.filter(c => 
    adminCarouselCompanyIds.includes(c.id) && !hasResultsDeclared(c)
  );

  // Only show companies in the carousel where results are not declared
  const carouselCompanies = adminCarouselCompanies;
          ...
          >
            ...
            <div className="flex flex-col md:flex-row gap-6">
              {hotBetsCompanies.map((company, idx) => {
                ...
                const stakes = companyStakes[company.id] || {};
